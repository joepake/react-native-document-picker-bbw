...
public class FilePickerDialog extends Dialog implements AdapterView.OnItemClickListener {
    ...
    public FilePickerDialog(Context context, DialogProperties properties, int primaryColor, int accentColor) {
        super(context);
        this.context = context;
        this.properties = properties;
        filter = new ExtensionFilter(properties);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (size == 0) {
            ...
            select.setTextColor(Color.argb(128, Color.red(accentColor), Color.green(accentColor), Color.blue(accentColor)));
        }
        ...
        mFileListAdapter.setNotifyItemCheckedListener(new NotifyItemChecked() {
            ...
            public void notifyCheckBoxIsClicked() {
                ...
                if (size == 0) {
                    ...
                    select.setTextColor(accentColor);
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        if (internalList.size() > i) {
            ...
            if (fitem.isDirectory()) {
                ...
                if (properties.selection_mode == DialogConfigs.SINGLE_MODE) {
                    AppCompatRadioButton rbSingleMode = view.findViewById(R.id.rbSingleMode);
                    rbSingleMode.performClick();
                } else {
                    AppCompatCheckBox cbMultiMode = view.findViewById(R.id.cbMultiMode);
                    cbMultiMode.performClick();
                }
            }
        }
    }
